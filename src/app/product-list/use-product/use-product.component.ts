    ...
    this.productService
      ...
      .subscribe((product: GetDetailedProduct) => {
        ...
        this.useProductForm = new FormGroup({
          researcherName: new FormControl({ value: this.user?.name, disabled: true }),
          date: new FormControl({ value: this.datepipe.transform(new Date(), 'dd/MM/yyyy - HH:mm'), disabled: true }),
          productName: new FormControl({ value: this.product?.name, disabled: true }),
          quantity: new FormControl(0),
